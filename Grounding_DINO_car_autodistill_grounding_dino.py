...
INPUT_DIR = Path("Car_hidream") 
OUTPUT_DIR = Path("car_results_autodistill")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
JSON_OUTPUT_PATH = OUTPUT_DIR / "grounding_dino_car.json"
...
ontology = CaptionOntology({
    ...
    "car bonnet front hood": "bonnet (front hood)",
    "car wheels": "wheels",
    "car doors": "doors",
    "car fender": "car fender",
    "car side windows": "side windows",
    "car rearview mirrors": "rearview mirrors",
    "car trunk rear compartment": "trunk (rear compartment)",
    "car taillights": "taillights",
    "car roof": "roof",
    "car front bumper": "front bumper",
    "car rear bumper": "rear bumper",
    "car grille front panel": "grille (front panel)"
})
...
for img_name in image_files:
    ...
    results = class_aware_nms(results, ontology, {"windshield": 0.3, "headlights": 0.4, "wheels": 0.5, "rearview mirrors": 0.5, "doors": 0.3})
    ...
    results_list.append({"image_id": img_name, "components": components, "wheel_count": components.get("wheels", 0), "average_confidence": round(float(np.mean(confidences)), 2) if confidences else 0.0, "total_detections": len(img_detections), "detections": img_detections})
